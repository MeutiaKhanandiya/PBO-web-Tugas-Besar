...
if( empty( $_SESSION['id_user'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$no_nota = $_REQUEST['Number'];
		$jenis = $_REQUEST['Number Music'];
		$nama = $_REQUEST['Code Music'];
		$bayar = $_REQUEST['Judul Music'];
		$kembali = $_REQUEST['Harga VVIP'];
		$total = $_REQUEST['Tanggal'];
		$id_user = $_SESSION['Tindakan'];

		$sql = mysqli_query($koneksi, "INSERT INTO transaksi( number , number music, code music, judul music, harga vvip, tanggal, tindakan) VALUES('$number', '$number_music', '$code music', '$judul music', '$harga vvip', '$tanggal', NOW(), '$tindakan')");

		if($sql == true){
			header('Location: ./Login.php?hlm=transaksi');
			die();
		} else {
...
<form method="post" action="" class="form-horizontal" role="form">
	<div class="form-group">
		<label for="no_nota" class="col-sm-2 control-label">Number Music</label>
		<div class="col-sm-3">

		<?php

			$sql = mysqli_query($koneksi, "SELECT no_nota FROM transaksi");
				echo '<input type="text" class="form-control" id="number_music" value="';
			...
			while(list($no_nota) = mysqli_fetch_array($sql)){
				...
				echo '"name="no_nota" placeholder="Number Music" readonly>';
  ...
  $(document).ready(function(){ /* PREPARE THE SCRIPT */
    ...
    $("#jenis").change(function(){ /* TRIGGER THIS WHEN USER HAS SELECTED DATA FROM THE SELECT FIELD */
      ...
      $.ajax({ /* START AJAX */
		...
		<label for="jenis" class="col-sm-2 control-label">Jenis Music</label>
		<div class="col-sm-3">
			<select name="jenis" class="form-control" id="jenis" required>
				<option value="" disable>--- Pilih Jenis Music ---</option>
			<?php
	...
	<div class="form-group">
		<label for="biaya" class="col-sm-2 control-label">Harga VVIP</label>
		<div class="col-sm-3">
		<select name"biaya">
			<option value="20000">POP - Rp 20000</option>
			<option value="50000">REMIX - Rp 50000</option>
		</select>
		</div>
	</div>
	<div class="form-group">
